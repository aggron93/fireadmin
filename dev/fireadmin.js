...
(function(window, document){
  ...
  */
  ...
  // What to do here?
  Fireadmin.prototype.auth = function (){
    return this;
  }
   ...
   * });
   */
  Fireadmin.auth.prototype.email = function(loginData, successCb, errorCb){
    var self = this;
  ...
  Fireadmin.prototype.login = function(loginData, successCb, errorCb){
    var self = this;
    //[TODO] Accept all kinds of login data
  };
  Fireadmin.login.prototype.withEmail = function(loginData, successCb, errorCb){
    var self = this;
  };
  Fireadmin.login.prototype.withGithub = function(loginData, successCb, errorCb){
    var self = this;
  };
  Fireadmin.login.prototype.withGoogle = function(loginData, successCb, errorCb){
    var self = this;
  };
  Fireadmin.login.prototype.withTwitter = function(loginData, successCb, errorCb){
    var self = this;
  };
  Fireadmin.login.prototype.withFacebook = function(loginData, successCb, errorCb){
    var self = this;
  };
  Fireadmin.login.prototype.anonymously = function(successCb, errorCb){
    var self = this;
  };
  /**
   * Gets list of objects created by the currently logged in User.
   * @memberof Fireadmin#
   * @param {String | Array} listPath - The name or path of the list the objects will be grabbed from.
   * @param {Function} onSuccess - Function that runs when the list has been retrieved successfully
   * @param {Fireadmin~errorCb} onError - Function that runs if there is an error.
   */
  Fireadmin.prototype.list = function(listPath){
    return this.fbRef(listPath);
  }
  /**
   * Gets list of objects created by the currently logged in User.
   * @memberof Fireadmin.list#
   * @param {String} listName - The name of the list the objects will be grabbed from.
   * @param {Function} onSuccess - Function that runs when the list has been retrieved successfully
   * @param {Fireadmin~errorCb} onError - Function that runs if there is an error.
   * @example
   * // Signin User with email and password
   * fb.list.byCurrentUser("messages", function(messageList){
   *  console.log('List of messages by currently logged in user:', messageList);
   * }, function(err){
   *  console.error('Error getting message list:', err);
   * });
   */
  Fireadmin.list.prototype.byCurrentUser = function(successCb, errorCb) {
    var auth = this.getAuth();
    if(auth != null) {
      this.orderByChild('author').equalTo(auth.uid).on('value', function(listSnap){
  ...
  * });
  */
  Fireadmin.list.prototype.byUid = function(listPath, uid, successCb, errorCb) {
  ...
  */
  //???
  Fireadmin.list.prototype.count = function(listPath, successCb, errorCb){
    var self = this;
  ...
  Fireadmin.list.prototype.users = function(){
    // [TODO] Implement limiting on this incase it is large
    this.child('users').on('value', function(usersListSnap){
      handleCb(successCb, usersListSnap.val());
    }, function(err){
      handleCb(errorCb, err);
    });
  };
  ...
  * });
  */
  Fireadmin.count.prototype.users = function(successCb, errorCb){
    this.child('users').on('value', function(usersListSnap){
      handleCb(successCb, usersListSnap.numChildren());
    }, function(err){
      handleCb(errorCb, err);
  ...
  *
  */
  Fireadmin.count.prototype.onlineUsers = function(successCb, errorCb){
  ...
  *
  */
  Fireadmin.list.prototype.onlineUsers = function(successCb, errorCb){
    this.child('presence').on("value", function(onlineUserSnap){
      console.log();
      handleCb(successCb, onlineUserSnap.val());
    }, function(err){
      handleCb(errorCb, err);
    });
  };
  ...
  *
  */
  Fireadmin.account.prototype.byUid = function(uid, successCb, errorCb){
   ...
   *
   */
  Fireadmin.account.prototype.byEmail = function(email, successCb, errorCb){
